 ...
 *
 * Sube la imagen de un Servicio para Eventos (las cards verticales de la
 * sección Servicios usan esta foto como protagonista) — multipart/form-data,
 * campo "file".
 * - Requiere token de admin.
 * - Convierte a WebP con sharp y guarda en public/services/.
 * - Devuelve { url, path }; la URL se guarda en Service.image.
 */
...
export async function POST(req: Request) {
  ...
  try {
    ...
    try {
      ...
      // 900px de ancho: las cards de servicios son verticales (3:4)
      output = await sharp(buffer).resize({ width: 900, withoutEnlargement: true }).webp({ quality: 82 }).toBuffer();
    } catch {
